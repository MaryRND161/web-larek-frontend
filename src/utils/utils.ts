...
export function formatPrice(x: number | null) {
	return x === null ? 'Бесценно' : `${formatNumber(x)} синапсов`;
}

export function renderLoading(el: HTMLElement, isLoading: boolean) {
	const submit = el.querySelector('.card__button');
	if (isLoading) {
		submit.textContent = 'Купить';
	} else {
